...
import (
	...
	"github.com/kristiansantos/ms_first/initializer"
...
func cmdRun(cmd *cobra.Command, args []string) {
	if err := godotenv.Load("./configs/.env." + env); err != nil {
		panic(err)
	}

	os.Getenv("ENV")

	cfg, err := initializer.ReadEnvironments(env, version)
	...
	svr := initializer.NewServer(addr, port)
	log := logger.New()
	svr.Run(cfg, log)
